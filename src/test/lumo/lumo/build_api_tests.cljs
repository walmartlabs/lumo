...
(deftest lumo-308-test
  (let [out (path/join (test/tmp-dir) "lumo-308-test-out")
    ...
    (testing ":optimizations :none"
      (test/delete-out-files out)
      (is (nil? (build/build
                 (path/join root "test_check" "instrument")
                 {:main 'test-check.instrument.core
                  :verbose true
                  :optimizations :none
                  :output-to out}
                 (env/default-compiler-env))) "It should compile without WARNING")
      (test/delete-out-files out))))
